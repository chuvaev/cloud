...
import chuvaev.cloud.api.remote.FileRemoteService;
...
public class FileRemoteServiceBean implements FileRemoteService {
    ...
    @Override
    @SneakyThrows
    public void createTextFile(@Nullable final String filaName, @Nullable final String text){
        if (text == null) return;
        writeData(filaName, text.getBytes());
    }
    ...
    public void writeData(@Nullable final String fileName, byte[] data){
        ...
        final Node file = root.addNode("nt:file");
